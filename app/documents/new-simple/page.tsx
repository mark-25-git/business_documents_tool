...
const documentTypeLabels: Record<DocumentType, string> = {
  ...
  const pdfDocument = React.useMemo(() => (
    <DocumentPDF
      ...
      imageUrl={imageUrl}
    />
  ), [type, docNumber, docNumberPreview, date, selectedCustomer, previewItems, imageUrl]);
  ...
  const handleAddCustomer = async () => {
    ...
    const temp = { ...newCustomer, id: "TEMP-" + Date.now(), createdAt: new Date().toISOString() };
                ...
                <div className="p-4 bg-gray-50 rounded-md border border-gray-200">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <div className="text-gray-600 text-sm mt-1">{selectedCustomer.phone}</div>
                      <div className="text-gray-600 text-sm">{selectedCustomer.address}</div>
                    </div>
                    <button 
                      onClick={() => {
